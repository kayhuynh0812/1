...
var jAlbumController = (function () {
	...
	if(!jAlbumGlobals1649786692587.credits) $jAlbum('#jAlbum-footer .center').hide(); // hide credits in footer if disabled

	/** go back also on background click possible **/
	$jAlbum("#Responsive" + jAlbumGlobals1649786692587.uniqueId + ", .jAlbum #jAlbum-footer, .jAlbum #jAlbum-footer .center, .jAlbum #jAlbum-footer .left, .jAlbum #jAlbum-footer p, .jAlbum #jAlbum-content, .jAlbum #print").click(function(event) {
	...
	function getWidth(){
		return $jAlbum("#Responsive" + jAlbumGlobals1649786692587.uniqueId).width();
	}
	...
	function getMaxLetterParam(){
		return (jAlbumGlobals1649786692587.imgBoxSize / 25) * 3 + (16 - jAlbumGlobals1649786692587.textSize) * 2 - 8;
	}

	function getImgBoxPadding(){
		return jAlbumGlobals1649786692587.mobile ? 15 : 25;
	}

	function getBoxSize(){
		return jAlbumGlobals1649786692587.mobile ? jAlbumGlobals1649786692587.imgBoxSize - 25 : jAlbumGlobals1649786692587.imgBoxSize;
	}

	function getNumImgsInRow(innerWidth){
	    var number = Math.round((innerWidth)/BOX_SIZE);
	    return (number < 1) ? 1 : (number > jAlbumGlobals1649786692587.maxImgInRow) ? jAlbumGlobals1649786692587.maxImgInRow : number;
	}
	...
	function setHashPathAndSlide(currentSlide){
		...
		if(currentPath == "" && jAlbumGlobals1649786692587.online) history.pushState('', document.title, window.location.pathname);
	...
	function preloadCurrSlides(){
		for(var i = 0; i < slides.length; i++){
			preloadImage(jAlbumGlobals1649786692587.jAlbumURL + currentPath + slides[i].path);
		}
	}
	...
	function preloadBeforeAndAfter(index, number){ // number desides how many images should be loaded in both directions
		...
		for(var i = start; i < end; i++){
			if (slides[i].category == 'image') {
				jAlbumLibrary.preloadImg(jAlbumGlobals1649786692587.jAlbumURL + currentPath + slides[i].path);
			}
		}
	}
	...
	function getAlbumData(path){
		var obj = jAlbumGlobals1649786692587.dataTree; // jAlbum dataTree
	...
	function arrange() {
		...
		for(var i = 0; i < 2; i++){
			...
			$jAlbum("#Responsive" + jAlbumGlobals1649786692587.uniqueId + " #jAlbum-content").css("padding", '' + topBottomPadding + 'px 2.5%');
	...
	function initFolder(index, json){
			...
			if(numOfImg < 3){ // 0 is the first one
				switch (object.category) {
					...
					case 'image':
						...
						var img = jAlbumLibrary.img("img-folder-" + numOfImg + "-" + index, jAlbumGlobals1649786692587.jAlbumURL + imgPath);
						...
						if(!jAlbumGlobals1649786692587.mobile){
						    img.hover(function(){
						    	mainBox.css({
						    		"-webkit-transform": jAlbumGlobals1649786692587.imgHoverScaleFactor,
							    	"-moz-transform": jAlbumGlobals1649786692587.imgHoverScaleFactor,
							    	"-o-transform": jAlbumGlobals1649786692587.imgHoverScaleFactor,
							    	"transform": jAlbumGlobals1649786692587.imgHoverScaleFactor
						    	});
							}, function(){
								mainBox.css({
		...
		if(numOfImg == 0){
		    ...
		    img.attr("src", jAlbumGlobals1649786692587.jAlbumURL + "res/img/folder.png");
		    img.css("position", "absolute");

			// add style
			if(jAlbumGlobals1649786692587.showFolderName){
				img.css("width", "60%");
				img.css("margin", "20%");
			}else{
				img.css("width", "80%");
		...
		if(jAlbumGlobals1649786692587.showFolderName){
			...
			if(jAlbumGlobals1649786692587.folderImgCount){
	    ...
	    $jAlbum("#Responsive" + jAlbumGlobals1649786692587.uniqueId + " #jAlbum-content").append(coverBox);
	...
	function arrangeFolders(mainBoxSize, imgSize){
		...
		if(jAlbumGlobals1649786692587.folderTitleUp){
		...
		imgSize = jAlbumGlobals1649786692587.showFolderName ? imgSize * 0.5 : imgSize * 0.8;
		...
		$jAlbum.each(folders, function(index, folder) {
			...
			if(folder.width != undefined){
				...
				for(var i = 0; i < 3; i++){
					...
					var img = $jAlbum("#Responsive" + jAlbumGlobals1649786692587.uniqueId + " #img-folder-" + i + "-" + folder.index);
					var x = (i - 1) * - 10;
					var y = (i - 1) * - 10;
					var scaledWidth = 0;
					var scaledHeight = 0;
					...
					img.css("margin", ((mainBoxSize - scaledHeight)/2 - jAlbumGlobals1649786692587.imgBorderSize) + "px " + ((mainBoxSize - scaledWidth)/2 - jAlbumGlobals1649786692587.imgBorderSize) + "px");
				}
			}
		}); 
	}
	...
	function initImage(index, object){
	    ...
	    var img = jAlbumLibrary.img("img-thumb-" + index, jAlbumGlobals1649786692587.jAlbumURL + currentPath + object.thumb.path);
	    ...
	    if(object.category == 'video'){
		    ...
		    video_icon.attr("src", jAlbumGlobals1649786692587.jAlbumURL + "res/img/play.png");
	    ...
	    $jAlbum("#Responsive" + jAlbumGlobals1649786692587.uniqueId + " #jAlbum-content").append(box);
	...
	function arrangeImages(imgBoxSize, imgSize) {
		...
		$jAlbum.each(thumbs, function(index, thumb) {
	    	...
	    	var img = $jAlbum("#Responsive" + jAlbumGlobals1649786692587.uniqueId + " #img-thumb-" + thumb.index);
			...
			img.css("margin", ((imgBoxSize - scaledHeight)/2 - jAlbumGlobals1649786692587.imgBorderSize) + "px " + ((imgBoxSize - scaledWidth)/2 - jAlbumGlobals1649786692587.imgBorderSize) + "px");
	...
	function fullscreen(index){
	    ...
	    var play = $jAlbum("<div/>");
	    var thumbnail = slides[index].category == 'video' ? $jAlbum("<img src='" + jAlbumGlobals1649786692587.jAlbumURL + currentPath + slides[index].videothumb + "'>") : $jAlbum('<img>');
	    var description = $jAlbum("<div/>");

	    // hide description if disabled
	    if(!jAlbumGlobals1649786692587.showComments) description.hide();

	    thumbnail.css({"width": "100%", "height": "100%", "position": "absolute"});

	    if(!jAlbumGlobals1649786692587.mobile) thumbnail.css("opacity", "0");

	    description.unbind();

	    // handle navigation
		var fs = $jAlbum("#fullscreen");
		var em = $jAlbum("#Responsive" + jAlbumGlobals1649786692587.uniqueId)
		...
		switch (slides[index].category) {
			case 'image':
				img = jAlbumLibrary.img("img-fullscreen-" + index, jAlbumGlobals1649786692587.jAlbumURL + path);
				play.hide();
				break;
			case 'video':
				img = jAlbumLibrary.vid("img-fullscreen-" + index, jAlbumGlobals1649786692587.jAlbumURL + path);
				play.show();
				break;
		}
	...
	function arrangeFullscreen(){
		...
		if(tempSlideIndex !== null){
			...
			var imageBoxWidth = innerWidth - jAlbumGlobals1649786692587.slideMarginSize * 2 * innerWidth / 100;
			var imageBoxHeight = innerHeight - jAlbumGlobals1649786692587.slideMarginSize * 2 * innerHeight / 100;
